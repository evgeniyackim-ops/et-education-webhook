...
async function detectDialogflowIntent(sessionId, queryText) {
  ...
  try {
    ...
    return {
      ...
      isFallback: !!result.intentDetectionConfidence === false ? result.intent?.isFallback || false : result.intent?.isFallback || false,
...
async function buildDialogflowDiagnostics(testText) {
  const diagnostics = [];
  ...
  diagnostics.push(`Интент: ${result.intentName || 'не определён'}`);
  diagnostics.push(`Fallback: ${result.isFallback ? 'да' : 'нет'}`);
  diagnostics.push(`Confidence: ${typeof result.confidence === 'number' ? result.confidence : 'нет данных'}`);
  diagnostics.push(`Fulfillment: ${result.fulfillmentText ? result.fulfillmentText : 'пустой ответ'}`);
...
async function handleTelegramTextMessage(req, message) {
  ...
  if (normalized.startsWith('/diag')) {
    const testText = text.replace(/^\/diag(@\w+)?/i, '').trim() || '/command1';
    const diagText = await buildDialogflowDiagnostics(testText);
    await sendTelegramMessage(chatId, diagText, getMainMenuReplyMarkup(baseUrl));
    return;
  }
